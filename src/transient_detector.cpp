...
std::vector<float> AnalyzeGain(const float* in, const uint32_t len, const uint32_t maxPoints, bool useRms,
                               std::vector<float>* lowBand, std::vector<float>* highBand) {
    vector<float> res;
    ...
    for (uint32_t pos = 0; pos < len; pos += step) {
        ...
        if (lowBand || highBand) {
            const uint32_t half = std::max<uint32_t>(1, step / 2);
            const float low = useRms ? calculateRMS(in + pos, half) : calculatePeak(in + pos, half);
            const float high = useRms
                ? calculateRMS(in + pos + (step - half), half)
                : calculatePeak(in + pos + (step - half), half);
            if (lowBand) {
                lowBand->emplace_back(low);
            }
            if (highBand) {
                highBand->emplace_back(high);
            }
        }
    }
    return res;
