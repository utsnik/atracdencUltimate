...
static uint32_t Atrac3CliBitrateToBps(uint32_t bitrateKbps)
{
    if (bitrateKbps == 0) {
        return 0;
    }
    // Keep true LP2 when the CLI uses the common "132" shorthand.
    if (bitrateKbps == 132) {
        return 132300;
    }
    return bitrateKbps * 1024;
}
                                 ...
                                 TAtracProcessorPtr* atracProcessor)
{
    ...
    *totalSamples = wavIO->GetTotalSamples();
    // ATRAC3 path keeps one look-ahead frame in the encoder pipeline.
    // Reserve ceil(input/1024) + 1 frames in container metadata.
    const uint64_t numFrames = ((*totalSamples + 1023) / 1024) + 1;
    if (numFrames >= UINT32_MAX) {
        std::cerr << "Number of input samples exceeds output format limitation,"
            "the result will be incorrect" << std::endl;
    }
    ...
    const bool useJointStereo = encoderSettings.UseJointStereo();
                                  ...
                                  const char* advancedOpt)
{
    *totalSamples = wavIO->GetTotalSamples();
    ...
    atracProcessor->reset(new TAt3PEnc(std::move(omaIO), numChannels, 2048, settings));
}
...
int main_(int argc, char* const* argv)
{
    ...
    static struct option longopts[] = {
        ...
        { "ml-hints", no_argument, NULL, O_ML_HINTS},
        { "decision-log", required_argument, NULL, O_DECISION_LOG},
        { "parity", no_argument, NULL, O_PARITY},
        { "parity-search", no_argument, NULL, O_PARITY_SEARCH},
        { "quality-v10", no_argument, NULL, O_QUALITY_V10},
        { "quality-v10-stable", no_argument, NULL, O_QUALITY_V10_STABLE},
        { "stereo-exp", no_argument, NULL, O_STEREO_EXP},
    ...
    while ((ch = getopt_long(argc, argv, "e:dhi:o:m", longopts, NULL)) != -1) {
        switch (ch) {
            ...
            case O_ML_HINTS:
                ...
                break;
            case O_DECISION_LOG:
                decisionLogFile = optarg;
                break;
            case O_PARITY:
                enableParityAnalysis = true;
                break;
            case O_PARITY_SEARCH:
                enableParitySearch = true;
                break;
            case O_QUALITY_V10:
                enableQualityV10 = true;
                break;
            case O_QUALITY_V10_STABLE:
                enableStabilityMode = true;
                break;
            case O_STEREO_EXP:
                enableStereoExp = true;
                break;
            case O_STEREO_BALANCE_EXP:
                enableStereoBalanceExp = true;
                break;
            case O_GAIN_EXP:
                enableGainExp = true;
                break;
            ...
            case O_START_FRAME:
                ...
                break;
            case O_MAX_FRAMES:
                maxFrames = (uint32_t)checkedStoi(optarg, 0, INT32_MAX, 0);
                break;
            default:
                printUsage(myName);
                return 1;
    ...
    try {
        switch (mode) {
            case E_ENCODE:
	        {
                ...
                using NAtrac1::TAtrac1Data;
                NAtrac1::TAtrac1EncodeSettings encoderSettings(bfuIdxConst, false, windowMode, winMask);
            ...
            case (E_ENCODE | E_ATRAC3):
            {
                ...
                std::ostream* decisionOut = nullptr;
                static std::ofstream decisionLogStream;
                ...
                if (!decisionLogFile.empty()) {
                    decisionLogStream.open(decisionLogFile);
                    if (!decisionLogStream) {
                        cerr << "Cannot open decision-log file: " << decisionLogFile << endl;
                        return 1;
                    }
                    decisionOut = &decisionLogStream;
                }
                NAtrac3::TAtrac3EncoderSettings encoderSettings(Atrac3CliBitrateToBps(bitrate), noGainControl,
