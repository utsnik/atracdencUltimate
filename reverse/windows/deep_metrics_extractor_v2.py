"""
deep_metrics_extractor_v2.py

Per-frame LP2 (132kbps ATRAC3) decoder-state dump from at3tool.exe.
Hooks confirmed LP2-path functions. Exports CSV + JSONL.

Confirmed binary:
  SHA256: 705B40CFC26DD1227D7D937C9102FBB9C4375C9CC9EF10F589CE0BA11EADCA1B
  Version: SCEI ATRAC3plus Codec Tool 3.0.0.0 / Library 1.02
...
RVAS = {
    ...
    "40d20":       0x40d20,  # FUN_00440d20: energy helper
    "energies_fn": 0x3ec60,  # FUN_0043ec60: segment energy clear/init
}
...
send({type: 'log', msg: 'JS Script loaded. Base: ' + base});

// ── Hook: Frame entry ───────────────────────────────────────────────────────
// We use RVA_ATRAC3_ENTRY (0x4d20)
Interceptor.attach(base.add(RVA_ATRAC3_ENTRY), {
    onEnter: function() { 
        frame_counter++; 
        lp2_call_in_frame = 0;
        if (frame_counter % 500 === 0) {
            send({type: 'log', msg: 'Processed frame ' + frame_counter});
        }
    }
});

// ── Hook: Bit count collectors ──────────────────────────────────────────────
var tonal_bits_per_call = [];
Interceptor.attach(base.add(RVA_TONAL_BITS), {
    onLeave: function(retval) { 
        tonal_bits_per_call.push(retval.toInt32()); 
    }
});

var gain_bits_per_call = [];
Interceptor.attach(base.add(RVA_GAIN_BITS), {
    onLeave: function(retval) { 
        gain_bits_per_call.push(retval.toInt32()); 
    }
});

var extra_bits_per_call = [];
Interceptor.attach(base.add(RVA_EXTRA_BITS), {
    onLeave: function(retval) { 
        extra_bits_per_call.push(retval.toInt32()); 
    }
});

// ── Hook: Tonal Candidate Finder (3f110) ──────────────────────────────────
var tonal_candidates_per_call = [];
Interceptor.attach(base.add(RVA_3F110), {
    onLeave: function(retval) {
        tonal_candidates_per_call.push(retval.toInt32());
    }
});

// ── Hook: Bit cost evaluator (e8c0) ──────────────────────────────────────────
var e8c0_costs = [];
Interceptor.attach(base.add(RVA_E8C0), {
    onEnter: function(args) {
        this.cost_ptr = args[5];
    },
    onLeave: function() {
        try {
            e8c0_costs.push(this.cost_ptr.readS32());
        } catch(e) { }
    }
});

// ── Hook: Main Encoder (lp2_main) ─────────────────────────────────────────────
Interceptor.attach(base.add(RVA_LP2_MAIN), {
    onEnter: function(args) {
        var ctx = args[2];
        this.ctx = ctx;
    ...
    onLeave: function(retval) {
        ...
        // Gather weight_idx and gain_points_per_band (read first 4 as requested)
        var weights = [];
        var gain_points = [];
        var tonals_per_band = [];
        for (var i = 0; i < 4; i++) {
...
def run_extraction(sample_path):
            ...
            if isinstance(payload, dict):
                if payload.get('type') == 'row':
                    ...
                    if len(results) % 100 == 0:
                        print(f"  [{sample_name}] {len(results)} rows collected...", end='\r')
    ...
    # Start at3tool
    cmd = [AT3TOOL, "-e", "-br", str(BITRATE), sample_path, "temp.at3"]
    pid = frida.spawn(cmd)
    ...
    while True:
        try:
            # os.kill(pid, 0) is better than Process.is_alive
            os.kill(pid, 0)
        except OSError:
            break
        time.sleep(0.5)

    # Final wait for buffers
    time.sleep(2)
